...
export function createHatchDialog({friends,reduced,onMeet}){
 ...
 $('draw-meet').addEventListener('click',()=>{if(!last)return;$('draw-dialog').close();onMeet(last.id,last.birdId);});
 return {show(result){
   ...
   $('draw-copy').textContent=result.duplicate?'같은 종류의 새 달걀이 이사 왔어요! 따로 진화시킬 수 있어요. 별조각 40개와 꾸미기 조각 1개도 받았어요.':'작은 발로 총총, '+egg.name+'가 이사 왔어요.';
