...
export function createVillageLife({residents,root,data,container,reduced,project}){
 ...
 const anchor=r=>data.layout['bird-'+r.index]||{x:r.x,z:r.z};
 function choose(r){
 ...
 return {update(dt,editing,selected,speaking=false){
  ...
  residents.forEach((r,i)=>{
   ...
   mesh.userData.eyes?.forEach(({eye,shine})=>{eye.scale.y=sleeping?.013:.065;shine.visible=!sleeping&&!mesh.userData.birdStage;});
