...
const validRecord=text=>{try{const d=JSON.parse(text);return d&&typeof d==='object'&&((Array.isArray(d.owned)&&d.owned.some(i=>Number.isInteger(i)&&i>=0&&i<18))||(Number.isInteger(d.favorite)&&d.favorite>=0&&d.favorite<18));}catch{return false;}};
export const freshVillage=(random=Math.random)=>{const starter=Math.floor(random()*6);return {favorite:starter,owned:[starter],shards:300,tickets:0,layout:{},nicknames:Array(18).fill(''),fragments:Array(18).fill(0),outfits:Array(18).fill('none'),daily:{day:localDay(),completed:0,claimed:false},draws:0,garden:'peach',hearts:Array(18).fill(0),greeted:Array(18).fill(''),matches:0,wins:0};};
...
export function readVillage(storage){
  ...
  try{
    ...
    const layout={};for(const [key,p] of Object.entries(data.layout||{}).slice(0,80))if(/^(house|tree|flower|pond|fountain|arena|bench|fence|egg)-\d+$/.test(key)&&Number.isFinite(p?.x)&&Number.isFinite(p?.z)&&Math.abs(p.x)<=12&&Math.abs(p.z)<=9)layout[key]={x:p.x,z:p.z};
    ...
    if(!validRecord(raw))Object.defineProperty(restored,'recovered',{value:true});return restored;
...
export function drawEgg(data,random=Math.random){
 ...
 return {id,duplicate,refund:duplicate?DUPLICATE_REFUND:0,payment};
}
